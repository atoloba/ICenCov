...
#'
#' @details
#' This function adapts function Aintmap from the \pkg{interval} package
...
Aintmap2 <- function(L,R, Lin=TRUE, Rin=TRUE, xmin=0, xmax=max(R), keepall=FALSE){
  ...
  rmove <- ifelse(cLR[-length(cLR)] == cLR[-1] &
                    ...
                    cLRin[-length(cLR)] == !cLRin[-1], TRUE, FALSE)
  rmove <- c(rmove, FALSE)
  cLR <- cLR[!rmove]
  cLeq1.Req0 <- cLeq1.Req0[!rmove]
  cLRin <- cLRin[!rmove]

  # Construction of intervals: e.g. case of Lin=TRUE, Rin=TRUE
  ...
  for (i in 1:n){
    intmapLe <- intmapL
    intmapRe <- intmapR
    if(Lin[i]==TRUE) intmapLe[intmapLin] <- intmapLe[intmapLin]+eps
    if(Rin[i]==TRUE) intmapRe[intmapRin] <- intmapRe[intmapRin]-eps
  ...
  if(!keepall){
    ...
    if(length(idel) > 0){
      ...
      A <- A[, -idel, drop = FALSE]
      intmap <- intmap[, -idel, drop = FALSE]
      attr(intmap, "LRin") <- LRin[, -idel, drop = FALSE]
    }
  }
...
#'
#' @param L,R Numeric vectors of left/right endpoints of the observed intervals.
#' @param Lin,Rin Logical scalar or length-\eqn{n} vectors indicating whether the
#'   left/right endpoints are included.
#' @param keepall Logical; if \code{FALSE} (default), remove partition intervals
#'   with zero contribution (columns with all zeros).
#' @param accu Convergence tolerance for the self-consistency iterations (default \code{1e-10}).
#'
#' @details
#' This function is adapted from the \pkg{interval} package to operate on
#' augmented Turnbull partitions.
#'
#' @seealso \code{\link[interval]{icfit}}
#' @export
icfit2 <- function(L, R, Lin = TRUE, Rin = TRUE, keepall = FALSE, accu = 1e-10){
