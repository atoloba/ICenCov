...
#' @param zmin Non-negative numeric. Lower bound of the support for `z`. Default is 0.
#' @param zmax Optional numeric. Upper bound of the support for `z`. If `NULL`, only `zmin` is enforced.
#' @param rini Optional function. A function that generates the initial gap after `zmin`.
#'    If \code{NULL}, no initial gap is added (i.e., 0).
...
censoring <- function(z, zmin = 0, zmax = NULL, rini = NULL, rfun, dec = NULL){
  ...
  if (zmin < 0) stop("`zmin` must be non-negative")
  if (!is.null(zmax)) {
  ...
  gaps <- gap_generator(n * max(1, ceiling(max(z))))
  ...
  # Diagnostic warning
  iwarning <- NULL
  if (any((zl - z > .Machine$double.eps^0.5) | (zr - z < -.Machine$double.eps^0.5))) {
